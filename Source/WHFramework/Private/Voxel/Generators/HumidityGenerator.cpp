...
void HumidityGenerator::GenerateHumidity(AVoxelChunk* chunk,UVoxelModule* info)
{
	const int32 times = 3;
	int32 cystalSize  = 16;
	...
	for (int d = 0; d < times; ++d)
	{
		for (int i = 0; i < UVoxelModule::Get().GetWorldData().ChunkSize.X; ++i)
		{
			for (int j = 0; j < UVoxelModule::Get().GetWorldData().ChunkSize.Y; ++j)
			{
				...
				float value = FMath::Clamp(UVoxelModule::Get().GetNoise2D(pf + (chunk->GetIndex().ToVector2D() / cystalSize), FVector(1.f, 1.f, 1.f), 201, true),0.0f,1.0f);
	
				chunk->GetTopography(FIndex(i, j)).Humidity += value / times;
			}
		}
		cystalSize*=2;
	}
