...
bool FVoxelInventoryTransaction::SetSilent(UAbilityInventorySlotBase& S, const FAbilityItem& Expected, const FAbilityItem& Next)
{
	...
	if (!S.GetInventory() || !S.IsEnabled() || !S.GetItem().Equal(Expected) || Next.Count < 0)
		return false;
	FAbilityItem N = Next.Count == 0 ? FAbilityItem::Empty : Next;
	if (N.Count > 0 && (!N.ID.IsValid() || !S.MatchItemLimit(N, true) || !S.MatchItemSplit(N, false) || N.Count > S.GetMaxVolume(N) || N.Level < 0 ||
	                    N.Level > S.GetMaxLevel(N)))
		return false;
	N.Payload = &S;
	N.Handle = N.Match(Expected) ? Expected.Handle : FGameplayAbilitySpecHandle();
	S.GetItem() = N;
	return true;
}
...
void FVoxelInventoryTransaction::Notify(UAbilityInventorySlotBase& S, const FAbilityItem& Previous)
{
	...
	if (S.GetItem().ID != Old.ID || S.GetItem().Level != Old.Level)
	{
		...
		if (auto Agent = S.GetInventory()->GetOwnerAgent())
			Agent->OnPreChangeItem(Old);
