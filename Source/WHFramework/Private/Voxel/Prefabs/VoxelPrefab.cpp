...
bool AVoxelPrefab::SetPreviewData(const FVoxelPrefabSaveData& Value, double Size, FString& Error)
{
	...
	{
		if (Error.IsEmpty())
			Error = TEXT("Prefab preview registry is unavailable");
		return false;
	}
	...
	{
		if (Error.IsEmpty())
			Error = TEXT("Prefab preview rendering resources are unavailable");
		return false;
	}
	...
	for (const FVoxelSectionMeshResult& Result : Results)
	{
		...
		{
			if (Batch.Mesh.Triangles.IsEmpty())
				continue;
...
void AVoxelPrefab::LoadData(const FParameter& Value, EPhase Phase)
{
	...
	{
		UE_LOG(LogTemp, Error, TEXT("Prefab preview load failed: %s"), Data ? *Error : TEXT("wrong parameter type"));
	}
}
