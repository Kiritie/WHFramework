...
{
    Error.Reset();
    if (Value.Cells.Num() > 4096)
    {
        Error = TEXT("Prefab exceeds 4096 cells");
        return false;
    }
    ...
    for (const FVoxelPrefabCell& Cell : Value.Cells)
    {
        ...
        if (!VoxelCoord::IsValid(Cell.Offset) || Cell.Item.Count != 1 ||
    ...
    if (Sections.Num() > 32)
    {
        Error = TEXT("Prefab spans more than 32 local sections");
        return false;
    }
