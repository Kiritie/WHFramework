...
uint64 FVoxelMeshBuffers::Bytes() const
{
	return uint64(Vertices.Num()) * sizeof(FVector) + uint64(Normals.Num()) * sizeof(FVector) + uint64(Triangles.Num()) * 4 +
	       uint64(UV0.Num() + UV1.Num() + UV2.Num()) * sizeof(FVector2D) + uint64(Colors.Num()) * sizeof(FLinearColor) +
	       uint64(Tangents.Num()) * sizeof(FProcMeshTangent);
}
bool FVoxelMeshBuffers::Validate() const
{
	int32 N = Vertices.Num();
	if (N != Normals.Num() || N != UV0.Num() || N != UV1.Num() || N != UV2.Num() || N != Colors.Num() || N != Tangents.Num() || Triangles.Num() % 3)
		return false;
	for (int32 I : Triangles)
		if (I < 0 || I >= N)
			return false;
	for (int32 I = 0; I < N; ++I)
		if (Vertices[I].ContainsNaN() || Normals[I].ContainsNaN() || !FMath::IsFinite(UV0[I].X) || !FMath::IsFinite(UV0[I].Y) || !FMath::IsFinite(UV1[I].X) ||
		    !FMath::IsFinite(UV1[I].Y) || !FMath::IsFinite(UV2[I].X))
			return false;
	return true;
}
uint64 FVoxelSectionMeshResult::Bytes() const
{
	uint64 N = 0;
...
namespace
{
FVector2D MakeFaceUV(uint8 Face, const FVector& Position, double VerticalExtent)
{
	if (Face < 2)
	{
		return FVector2D(Position.Y, VerticalExtent - Position.Z);
	}
	if (Face < 4)
	{
		return FVector2D(Position.X, VerticalExtent - Position.Z);
	}
...
struct FFaceKey
{
	bool Visible = false;
	uint32 Packed = 0;
	FVoxelRuntimeFaceRef Texture;
	EVoxelRenderGroup Group = EVoxelRenderGroup::Opaque;
	bool operator==(const FFaceKey& B) const
	{
		return Visible == B.Visible && Packed == B.Packed && Texture == B.Texture && Group == B.Group;
	}
};
bool Covered(const FVoxelResolvedShape& S, uint8 F, const FVoxelShapeQuad* Q)
{
	if (!Q)
		return (S.OcclusionMask & (1u << F)) != 0;
	int32 A = F / 2, U = (A + 1) % 3, V = (A + 2) % 3;
	double U0 = 1;
	double U1 = 0;
	double V0 = 1;
	double V1 = 0;
	for (const auto& P : Q->Vertices)
	{
		U0 = FMath::Min(U0, P[U]);
		U1 = FMath::Max(U1, P[U]);
		V0 = FMath::Min(V0, P[V]);
...
bool Visible(const FVoxelSectionSnapshot& S,
             const FVoxelRegistrySnapshot& R,
             const FVoxelShapeRegistry& H,
             FIntVector P,
             FVoxelBlockState A,
             uint8 Face,
             const FVoxelShapeQuad* Quad)
{
	FVoxelBlockState B;
	if (!S.TrySample(P + VoxelCoord::Direction(Face), B) || B.IsAir())
		return true;
	const auto* BD = R.Find(B.TypeId);
	const auto* AD = R.Find(A.TypeId);
	if (!BD || !AD)
		return true;
	...
	if (!BD->bOccludes && !SameTransparent)
		return true;
...
void AppendQuad(FVoxelMeshBuffers& O, const FVector* P, const FVector2D* UV, const FVoxelRuntimeFaceRef& T, EVoxelRenderGroup G, const FVector& Cell)
{
	...
	FVector Tangent = (P[1] - P[0]).GetSafeNormal();
	for (int32 I = 0; I < 4; ++I)
	{
		O.Vertices.Add(P[I] + Cell);
    ...
    const FVoxelSectionSnapshot& S, const FVoxelRegistrySnapshot& R, const FVoxelShapeRegistry& H, FVoxelSectionMeshResult& O, const std::atomic_bool* Cancel)
{
	if (S.Blocks.Num() != 4096)
		return false;
	for (uint32 P : S.Blocks)
		if (!R.IsValid(FVoxelBlockState::Unpack(P)))
			return false;
	FVoxelSectionMeshResult T;
	T.Stamp = S.Stamp;
	TMap<uint32, int32> Groups;
	int32 TotalVertices = 0;
	auto Batch = [&](EVoxelRenderGroup G, uint16 Bank) -> FVoxelMeshBuffers&
	{
		uint32 K = (uint32(G) << 16) | Bank;
		if (const int32* I = Groups.Find(K))
			return T.Batches[*I].Mesh;
		FVoxelRenderBatch B;
		B.Group = G;
		B.Bank = Bank;
		int32 I = T.Batches.Add(MoveTemp(B));
		Groups.Add(K, I);
		return T.Batches[I].Mesh;
	};
	auto OverBudget = [&]()
	{
		return TotalVertices > 262144;
	};
	for (uint8 F = 0; F < 6; ++F)
		for (int32 Slice = 0; Slice < 16; ++Slice)
		{
			...
			for (int32 Y = 0; Y < 16; ++Y)
				for (int32 X = 0; X < 16; ++X)
				{
					...
					P[U] = X;
					P[V] = Y;
					FVoxelBlockState B = FVoxelBlockState::Unpack(S.Blocks[VoxelCoord::Linear(P)]);
					if (B.IsAir())
						continue;
					const auto* D = R.Find(B.TypeId);
					if (D->Shape != EVoxelShapeKind::FullCube || !Visible(S, R, H, P, B, F, nullptr))
						continue;
					FFaceKey& K = Mask[X + 16 * Y];
					K.Visible = true;
					K.Packed = B.Pack();
					K.Group = D->RenderGroup;
					uint8 MF = FVoxelShapeRegistry::RotateFace(F, uint8((4 - (B.State & 3)) & 3));
					K.Texture = D->Face(B.State, MF);
				}
			for (int32 Y = 0; Y < 16; ++Y)
				for (int32 X = 0; X < 16;)
				{
					const auto K = Mask[X + 16 * Y];
					if (!K.Visible)
					{
						++X;
						continue;
					...
					int32 W = 1;
					int32 Ht = 1;
					while (X + W < 16 && Mask[X + W + 16 * Y] == K)
						++W;
					bool Stop = false;
					while (Y + Ht < 16 && !Stop)
					{
						for (int32 DX = 0; DX < W; ++DX)
							if (!(Mask[X + DX + 16 * (Y + Ht)] == K))
							{
								Stop = true;
								break;
							}
					...
					for (int32 I = 0; I < 4; ++I)
					{
						P[I] = FVector::ZeroVector;
						P[I][A] = Slice + (F % 2 == 0 ? 1 : 0);
						P[I][U] = XX[I];
						P[I][V] = YY[I];
					...
					for (int32 DY = 0; DY < Ht; ++DY)
						for (int32 DX = 0; DX < W; ++DX)
							Mask[X + DX + 16 * (Y + DY)].Visible = false;
					X += W;
				}
		}
	for (uint16 I = 0; I < 4096; ++I)
	{
		if ((I & 63) == 0 && Cancel && Cancel->load(std::memory_order_relaxed))
			return false;
		...
		if (D->Shape == EVoxelShapeKind::FullCube)
			continue;
		const auto* Shape = H.Find(D->Shape, B.State);
		if (!Shape)
			return false;
	...
	T.Batches.Sort(
	    [](const FVoxelRenderBatch& A, const FVoxelRenderBatch& B)
	    {
		    return A.Group != B.Group ? uint8(A.Group) < uint8(B.Group) : A.Bank < B.Bank;
	    });
