...
//初始化全局顶点
FVector2D UVoxelModuleStatics::GlobalVertex[4] = {};
FVector UVoxelModuleStatics::GlobalVertex3D[3] = {};

//初始化全局偏移
FVector2D UVoxelModuleStatics::GlobalOffset = FVector2D::ZeroVector;
FVector UVoxelModuleStatics::GlobalOffset3D = FVector::ZeroVector;

//初始化全局种子
int32 UVoxelModuleStatics::GlobalSeed = 107;

int32 UVoxelModuleStatics::hash11(int32 position)
{
...
int32 UVoxelModuleStatics::hash11WithGlobalSeed(int32 position){
	const uint32 BIT_NOISE1 = 0x85297A4D;
	const uint32 BIT_NOISE2 = 0x68E31DA4;
	const uint32 BIT_NOISE3 = 0x1B56C4E9;
...
FVector2D UVoxelModuleStatics::hash22(FVector2D position2D)
{
	FVector2D v(
...
int32 UVoxelModuleStatics::hash21(FVector2D position2D)
{
	return hash11(0x651A6BE1 * (int32)position2D.X + (int32)position2D.Y)%1024;
}

FVector UVoxelModuleStatics::hash33(FVector position)
{
	FVector v(
...
int32 UVoxelModuleStatics::hash31(FVector position3D)
{
	return hash11(position3D.X * 0x651A6BE6 - position3D.Y * 0xCB251062 + position3D.Z);
}

void UVoxelModuleStatics::setSeed(int32 seed){
	GlobalSeed = seed;
}

int32 UVoxelModuleStatics::randInt(FVector2D position){
	position = position*1024;
	return hash11WithGlobalSeed(0x651A6BE1 * (int32)position.X + (int32)position.Y)%1024;
}
	
float UVoxelModuleStatics::rand(FVector2D position){
	position = position*1024;
	return hash11WithGlobalSeed(0x651A6BE1 * (int32)position.X + (int32)position.Y)%1024/1024.0f;
}

float UVoxelModuleStatics::grad(FVector2D vertex, FVector2D position2D)
{
	return FVector2D::DotProduct(vertex, position2D);
}

float UVoxelModuleStatics::grad_f(FVector2D vertex, FVector2D p){
...
float UVoxelModuleStatics::perlinNoise(FVector2D p)
{
	p = GlobalOffset + p;
...
//TODO
float UVoxelModuleStatics::valueNoise(FVector2D position2D)
{
...
float UVoxelModuleStatics::simplexNoise(FVector2D p)
{
  p = p + GlobalOffset;
...
//TODO
float UVoxelModuleStatics::simplexNoise(FVector p){
 	p = p + GlobalOffset3D;
...
void UVoxelModuleStatics::prehandlePerlinNoise(FVector2D position2D, int32 crystalSize){
	FVector2D pi = FVector2D(floor(position2D.X / crystalSize), floor(position2D.Y / crystalSize));
	FVector2D vertex[4] = { {pi.X,pi.Y},{pi.X + 1,pi.Y},{pi.X,pi.Y + 1},{pi.X + 1,pi.Y + 1} };

	for(int32 i = 0;i<4;++i)
	GlobalVertex[i]=UVoxelModuleStatics::hash22(vertex[i]);
	
	GlobalOffset = (position2D-pi*crystalSize)/crystalSize;
}

//TODO
void UVoxelModuleStatics::prehandleValueNoise(FVector2D position2D, int32 crystalSize){

}

void UVoxelModuleStatics::prehandleSimplexNoise(FVector2D position2D, int32 crystalSize){
	GlobalOffset = position2D / crystalSize;
}

void UVoxelModuleStatics::prehandleSimplexNoise(FVector position,int32 crystalSize){
	GlobalOffset3D = position / crystalSize;
}

//二阶bezier曲线 t应[0.0f~1.0f]
FVector2D UVoxelModuleStatics::bezier(FVector2D p0,FVector2D p1,FVector2D p2,float t){
	return ((1-t)*(1-t)*p0+t*(1-t)*p1+t*t*p2);
}

//三阶bezier曲线 t应[0.0f~1.0f]
FVector2D UVoxelModuleStatics::bezier(FVector2D p0,FVector2D p1,FVector2D p2,FVector2D p3,float t){
	return bezier(p0,p1,p2,t)*(1-t)+bezier(p1,p2,p3,t)*t;
}

//三维坐标转一维坐标（压缩坐标）
uint64 UVoxelModuleStatics::Index(int32 x,int32 y,int32 z){
...
//一维坐标转三维坐标（解压坐标）
FVector UVoxelModuleStatics::UnIndex(uint64 index){
	const int32 offset = 16384;
	return FVector(
