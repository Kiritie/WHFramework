...
{
	if (B < 1 || B > 32 || N < 0 || N > 4096)
		return false;
	Bits = B;
	Count = N;
	Mask = (uint64(1) << B) - 1;
	Words.Empty();
	Words.SetNumZeroed(int32((uint64(B) * N + 63) / 64));
	return true;
}
uint32 FVoxelBitStorage::Get(int32 I) const
{
	check(I >= 0 && I < Count);
	uint64 P = uint64(I) * Bits;
	int32 W = int32(P / 64);
	uint32 S = uint32(P % 64);
	uint64 V = Words[W] >> S;
	if (S + Bits > 64)
		V |= Words[W + 1] << (64 - S);
