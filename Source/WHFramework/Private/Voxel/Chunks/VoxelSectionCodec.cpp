...
bool FVoxelSectionCodec::Encode(const FVoxelSectionStorage& S, TArray<uint8>& O)
{
	...
	uint8 B = 1;
	while ((uint32(1) << B) < uint32(P.Num()))
		++B;
	bool Direct = uint64(P.Num()) * 4 + uint64(B) * 512 >= 16384;
	FVoxelByteWriter W(20000);
	W.U32(0x31535856);
	W.U8(P.Num() == 1 ? 0 : Direct ? 2 : 1);
	if (P.Num() == 1)
		W.U32(P[0]);
	else if (Direct)
	{
	...
	else
	{
		...
		for (uint32 V : P)
			W.U32(V);
		FVoxelBitStorage Packed;
		if (!Packed.Reset(B, 4096))
			return false;
		for (int32 I = 0; I < 4096; ++I)
			Packed.Set(I, C[I]);
		W.U8(B);
		W.U16(uint16(Packed.GetWords().Num()));
		for (uint64 V : Packed.GetWords())
			W.U64(V);
	}
	return W.Finish(O);
}
bool FVoxelSectionCodec::Decode(TConstArrayView<uint8> B, TFunctionRef<bool(FVoxelBlockState)> Valid, FVoxelSectionStorage& O)
{
	...
	{
		uint32 V = R.U32();
		if (!Valid(FVoxelBlockState::Unpack(V)))
			return false;
		for (auto& X : D)
			X = V;
	}
	else if (Mode == 2)
	{
		...
		{
			V = R.U32();
			if (!R.IsValid() || !Valid(FVoxelBlockState::Unpack(V)))
				return false;
		}
	}
	else if (Mode == 1)
	{
		...
		{
			uint32 V = R.U32();
			if (!R.IsValid() || Seen.Contains(V) || !Valid(FVoxelBlockState::Unpack(V)))
				return false;
			Seen.Add(V);
			P.Add(V);
		}
		uint8 Bits = R.U8(), Expected = 1;
		while ((uint32(1) << Expected) < uint32(N))
			++Expected;
		int32 WN = R.U16();
		if (Bits != Expected || WN != (int32(Bits) * 4096 + 63) / 64)
			return false;
		TArray<uint64> Words;
		for (int32 I = 0; I < WN; ++I)
			Words.Add(R.U64());
		if (!R.IsValid())
			return false;
		FVoxelBitStorage V;
		if (!V.Import(Bits, 4096, Words))
			return false;
		for (int32 I = 0; I < 4096; ++I)
		{
			uint32 J = V.Get(I);
