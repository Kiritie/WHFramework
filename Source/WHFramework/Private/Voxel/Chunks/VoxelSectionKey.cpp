...
{
    check(D>0); const int32 Q=V/D; return Q-(V%D<0 ? 1 : 0);
}
...
FIntVector VoxelCoord::Local(const FIntVector& P) { return P-Origin(Section(P)); }
...
{ check(I<4096); return FIntVector(I&15,(I>>4)&15,(I>>8)&15); }
FIntVector VoxelCoord::Direction(uint8 F)
{
    check(F<6); FIntVector D(0,0,0); D[F/2]=(F%2==0?1:-1); return D;
}
FVoxelSectionKey VoxelCoord::Neighbor(const FVoxelSectionKey& K,uint8 F)
{
    const FIntVector D=Direction(F); return {K.X+D.X,K.Y+D.Y,K.Z+D.Z};
}
bool VoxelCoord::FromWorld(const FVector& P,double S,FIntVector& O)
{
    if(!FMath::IsFinite(S)||S<=0||P.ContainsNaN())return false;
    FVector G=P/S; const double M=VoxelBlock::MaxAbsCoordinate;
    if(!FMath::IsFinite(G.X)||!FMath::IsFinite(G.Y)||!FMath::IsFinite(G.Z)||
       G.X<-M||G.X>=M||G.Y<-M||G.Y>=M||G.Z<-M||G.Z>=M)return false;
    O=FIntVector(FMath::FloorToInt(G.X),FMath::FloorToInt(G.Y),FMath::FloorToInt(G.Z));return true;
}
