...
class UVoxelChunk;class UVoxelMaterialSet;class APlayerController;class UAbilityInventoryBase;class UVoxelAgentComponent;class UVoxelWorldGenerationProfile;
...
public:
    ...
    static UVoxelModule* Find(UWorld* World);
#if WITH_EDITOR
    virtual void OnDestroy() override;
#endif
    virtual UWorld* GetWorld()const override;
    bool BeginProjectMutation(){if(bMutating||!IsAuthority()||!IsReady())return false;bMutating=true;return true;}
    void EndProjectMutation(){bMutating=false;}
    void PublishProjectEdit(const FVoxelEditBatch& Batch){OnBlocksCommitted.Broadcast(Batch);}
    ...
    bool IsReady()const{return bool(Runtime)&&WorldState==EVoxelWorldState::Running;}
    float GetWarmupProgress()const;
    double BlockSize()const{return Manifest.BlockSizeCentimeters;}
    const FVoxelWorldManifest& GetManifest()const{return Manifest;}
    FGuid GetSessionId()const{return SessionId;}
    FVoxelWorldRuntime* GetRuntime(){return Runtime.Get();}
    const FVoxelWorldRuntime* GetRuntime()const{return Runtime.Get();}
    TSharedPtr<const FVoxelRegistrySnapshot,ESPMode::ThreadSafe> GetRegistry()const{return Registry.GetSnapshot();}
    TSharedPtr<const FVoxelShapeRegistry,ESPMode::ThreadSafe> GetShapes()const{return Shapes;}
    UVoxelMaterialSet* GetMaterialSet()const{return MaterialSet;}
    TSharedPtr<const FVoxelGenerationPipeline,ESPMode::ThreadSafe> GetGenerator()const{return Generator;}
    FVoxelWorldView* GetWorldView() const { return WorldView.Get(); }
    void ForceVoxelStreamingRefresh(){LastStreaming=-1;}
    bool IsProjectSceneSimulationEnabled() const {return bEnableProjectSceneSimulation;}
    ...
    const FVoxelWorldSaveData& GetWorldData()const{return *WorldData;}
    const FVoxelWorldBasicSaveData& GetWorldBasicData()const{return WorldBasicData;}
    ...
    void SetPendingCommitDirectory(const FString& Directory){PendingCommitDirectory=Directory;}
    ...
    UPROPERTY(BlueprintAssignable) FVoxelWorldInitialized OnWorldInitialized;
protected:
    ...
    UPROPERTY(EditAnywhere) FVoxelWorldBasicSaveData WorldBasicData;
    UPROPERTY(EditAnywhere) bool bAutoGenerate=false;
    UPROPERTY(EditAnywhere) EVoxelWorldMode WorldMode=EVoxelWorldMode::Default;
    UPROPERTY(Transient) EVoxelWorldState WorldState=EVoxelWorldState::None;
    UPROPERTY(EditAnywhere) TSubclassOf<UVoxelChunk> ChunkSpawnClass;
    UPROPERTY(EditAnywhere) TSoftObjectPtr<UVoxelMaterialSet> MaterialSetAsset;
    UPROPERTY(Transient) TObjectPtr<UVoxelMaterialSet> MaterialSet;
    UPROPERTY(EditAnywhere,Category="Voxel|Generation") TSoftObjectPtr<UVoxelWorldGenerationProfile> WorldGenerationProfileAsset;
    UPROPERTY(Transient) TObjectPtr<UVoxelWorldGenerationProfile> WorldGenerationProfile;
    UPROPERTY(EditAnywhere,Category="Voxel|Project") bool bEnableProjectSceneSimulation=false;
    UPROPERTY(Transient) TMap<FIntPoint,TObjectPtr<UVoxelChunk>> Columns;
    TUniquePtr<FVoxelWorldSaveData> WorldData;
private:
    struct FSource{TWeakObjectPtr<UObject>Owner;FVoxelStreamingSource Value;};
    struct FBreak{FIntVector Target;FVoxelBlockState Expected;double Began=0,LastPulse=0;};
    ...
    void PumpDecode();
    TMap<FString,TArray<uint8>> UnloadedSceneFiles,CapturedSceneFiles;
    ...
    FString LastSaveError,PendingCommitDirectory;
    double LastStreaming=-1;
    bool bMutating=false,bRemoteRunning=false;
};
