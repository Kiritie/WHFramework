...
{
    uint16 Kind=0,Schema=1;
    TArray<uint8> Payload;
    bool operator==(const FVoxelBlockEntityState&B)const{return Kind==B.Kind&&Schema==B.Schema&&Payload==B.Payload;}
};
...
struct WHFRAMEWORK_API FVoxelCellWrite{uint16 Index=0;FVoxelBlockState State;};
struct WHFRAMEWORK_API FVoxelEntityWrite{uint16 Index=0;bool bRemove=false;FVoxelBlockEntityState Value;};
struct WHFRAMEWORK_API FVoxelSectionPatch
{
    FVoxelSectionKey Key;
    uint64 FromRevision=0,ToRevision=0;
