...
{
public:
    FVoxelWorldRuntime(uint64 Epoch,bool bAuthority,TSharedRef<const FVoxelRegistrySnapshot,ESPMode::ThreadSafe> Registry,
    ...
    uint64 Epoch()const{return WorldEpoch;}
    bool Authority()const{return bAuthority;}
...
private:
    ...
    uint64 WorldEpoch,NextToken=1;
    bool bAuthority;
    TSharedRef<const FVoxelRegistrySnapshot,ESPMode::ThreadSafe> Registry;
    TSharedRef<const FVoxelGenerationPipeline,ESPMode::ThreadSafe> Generator;
    TMap<FVoxelSectionKey,TUniquePtr<FVoxelSection>> Sections;
};
