...
public:
    ...
    static bool EncodeSnapshots(const FVoxelSnapshotBatch& Batch,const FVoxelWorldManifest& Manifest,
        const FVoxelRegistrySnapshot& Registry,TArray<uint8>& Out);
    static bool DecodeSnapshots(TConstArrayView<uint8> Bytes,const FVoxelWorldManifest& Manifest,
        const FVoxelRegistrySnapshot& Registry,FVoxelSnapshotBatch& Out);
