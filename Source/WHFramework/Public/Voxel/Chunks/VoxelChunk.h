...
class UVoxelModule;class UVoxelMeshComponent;class UVoxelCollisionComponent;class UVoxelMaterialSet;
UCLASS()
...
public:
    ...
    FIntPoint GetColumn()const{return ColumnIndex;}
    ...
    const TMap<FGuid,TObjectPtr<AActor>>& GetSceneActors()const{return SceneActorMap;}
    bool bSceneReady=false,bSceneLoading=false,bSceneFailed=false;
protected:
    UPROPERTY(Transient) TObjectPtr<UVoxelModule> Module;
    UPROPERTY(Transient) TObjectPtr<AActor> Host;
    UPROPERTY(Transient) TMap<uint64,TObjectPtr<UVoxelMeshComponent>> Meshes;
    UPROPERTY(Transient) TMap<int32,TObjectPtr<UVoxelCollisionComponent>> Collisions;
    UPROPERTY(Transient) TMap<FGuid,TObjectPtr<AActor>> SceneActorMap;
