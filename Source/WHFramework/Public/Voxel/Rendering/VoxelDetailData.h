...
public:
	...
	UPROPERTY(EditAnywhere)
	TArray<FVector> SourceVerticesCm;

	UPROPERTY(EditAnywhere)
	TArray<int32> SourceTriangles;

	UPROPERTY(EditAnywhere)
	TSoftObjectPtr<UMaterialInterface> SourceMaterial;

	UPROPERTY(EditAnywhere)
	TArray<FBox> CollisionBoxesCm;

	UPROPERTY(EditAnywhere)
	FVoxelGridBox FootprintCells;

	UPROPERTY(EditAnywhere)
	bool bNearCollision = true;

	UPROPERTY(VisibleAnywhere)
	uint64 GeometryHash = 0;

	UPROPERTY(VisibleAnywhere)
	uint64 MetadataHash = 0;
