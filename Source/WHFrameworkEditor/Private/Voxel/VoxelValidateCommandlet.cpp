...
int32 UVoxelValidateCommandlet::Main(const FString& Params)
{
	...
	if (Set.IsValid())
	{
		for (const FVoxelMaterialBank& Bank : Set->Banks)
		{
			...
			if (!Texture || !Texture->Source.IsValid() || Texture->Source.GetSizeX() <= 0 || Texture->Source.GetSizeX() != Texture->Source.GetSizeY() ||
			    !Texture->SRGB)
				Issue(SetPath, TEXT("TextureArray"), FString::Printf(TEXT("Invalid source data in group %d bank %d"), int32(Bank.Group), Bank.Bank));
			if (!MIC || UMaterialEditingLibrary::GetMaterialInstanceTextureParameterValue(
			                MIC, TEXT("VoxelArray"), EMaterialParameterAssociation::GlobalParameter) != Texture)
	...
	for (const FAssetData& Entry : Found)
	{
		...
		if (!Asset->bRegisterBlock)
			continue;
		UVoxelData* A = Asset.Get();
		const FString Path = A->GetPathName();
		Error.Reset();
		if (!A->ValidateDefinition(true, Error))
			Issue(Path, TEXT("Definition"), Error);
		...
		if (Names.Contains(A->BlockName))
			Issue(Path, TEXT("BlockName"), TEXT("Duplicate stable name"));
		Names.Add(A->BlockName);
		IDs.Add(A->GetPrimaryAssetId());
		for (int32 Half = 0; Half < (A->Shape == EVoxelShapeKind::Door ? 2 : 1); ++Half)
		{
			...
			for (uint8 Face = 0; Face < 6; ++Face)
			{
				...
				if (!Texture || !Texture->Source.IsValid() || FaceTexture.FrameCount < 1 || FaceTexture.FrameCount > 256 || FaceTexture.FramesPerSecond < 0 ||
				    FaceTexture.FramesPerSecond > 60 || Texture->Source.GetSizeX() <= 0 || Texture->Source.GetSizeX() > 4096 ||
				...
				if (!Bank || uint32(Ref.Layer) + Ref.Frames > uint32(Bank->SliceCount) || Ref.Frames != FaceTexture.FrameCount ||
				    Ref.FPS != FaceTexture.FramesPerSecond)
				{
	...
	for (const FAssetData& Entry : Prefabs)
	{
		...
		Error.Reset();
		if (!Asset.IsValid())
			Issue(Entry.GetSoftObjectPath().ToString(), TEXT("PrefabLoad"), TEXT("Prefab failed to load"));
		else if (!bRegistryOK)
			Issue(Asset->GetPathName(), TEXT("PrefabBlocked"), TEXT("Cannot validate prefab references without a valid registry"));
		else if (!Asset->Validate(*Registry.GetSnapshot(), Error))
			Issue(Asset->GetPathName(), TEXT("Prefab"), Error);
	}
	...
	FString Text;
	if (!FJsonSerializer::Serialize(Report, TJsonWriterFactory<>::Create(&Text)) || !IFileManager::Get().MakeDirectory(*FPaths::GetPath(ReportPath), true) ||
	...
	UE_LOG(LogTemp, Display, TEXT("Validated %d voxel and %d prefab assets; %d issues. Report: %s"), Assets.Num(), Prefabs.Num(), Issues.Num(), *ReportPath);
