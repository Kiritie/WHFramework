...
{
    if(!Class||!FPackageName::IsValidLongPackageName(PackageName)){Error=TEXT("Invalid asset package path");return nullptr;}
    ...
    {
        if(!Existing->IsA(Class)){Error=TEXT("Asset class conflict: ")+ObjectPath;return nullptr;}
        Existing->Modify();return Existing;
    }
    UPackage* Package=CreatePackage(*PackageName);
    if(!Package){Error=TEXT("CreatePackage failed: ")+PackageName;return nullptr;}
    UObject* Asset=NewObject<UObject>(Package,Class,*Name,RF_Public|RF_Standalone|RF_Transactional);
    if(!Asset){Error=TEXT("NewObject failed: ")+ObjectPath;return nullptr;}
...
{
    if(!Asset){Error=TEXT("Cannot save a null asset");return false;}
    ...
    {Error=TEXT("Cannot create asset output directory: ")+File;return false;}
    FSavePackageArgs Args;Args.TopLevelFlags=RF_Public|RF_Standalone;Args.SaveFlags=SAVE_NoError;
    Asset->MarkPackageDirty();
    if(!UPackage::SavePackage(Package,Asset,*File,Args)){Error=TEXT("Package save failed: ")+File;return false;}
    return true;
}
