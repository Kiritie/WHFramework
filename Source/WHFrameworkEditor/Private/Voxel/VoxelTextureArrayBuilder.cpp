...
namespace
{
    struct FSource
    {
        FString Key;FVoxelFaceTexture Face;EVoxelRenderGroup Group;
    ...
    bool ReadFrames(FSource& S,int32 Tile,FString& Error)
    {
        ...
        {Error=TEXT("Source texture decoded pixels exceed 64 MiB: ")+S.Key;return false;}
        FImage Image,BGRA;
        if(!T->Source.GetMipImage(Image,0)||Image.NumSlices!=1||Image.SizeX<=0||Image.SizeX>4096||Image.SizeY!=int64(Image.SizeX)*S.Face.FrameCount)
        {Error=TEXT("Texture must be a square tile or declared vertical frame strip: ")+S.Key;return false;}
        ...
        for(int32 F=0;F<S.Face.FrameCount;++F)for(int32 Y=0;Y<Tile;++Y)for(int32 X=0;X<Tile;++X)
        {
...
bool FVoxelTextureArrayBuilder::Build(const TArray<UVoxelData*>& Assets,UVoxelMaterialSet& Set,const FVoxelArrayBuildOptions& O,FString& Error)
{
    ...
    if(Assets.IsEmpty()||!FPackageName::IsValidLongPackageName(O.OutputRoot)||O.TileSize<4||O.TileSize>256||!FMath::IsPowerOfTwo(O.TileSize)||O.MaxSlicesPerBank<1||O.MaxSlicesPerBank>2048)
    {Error=TEXT("Invalid array builder options");return false;}
    TArray<FSource> Sources;TMap<FString,int32> Lookup;TSet<FName> Names;
    for(UVoxelData* A:Assets)
    {
        if(!A||!A->ValidateDefinition(false,Error)||Names.Contains(A->BlockName))
        {if(Error.IsEmpty())Error=TEXT("Duplicate or invalid block asset");return false;}
        Names.Add(A->BlockName);
        for(int32 H=0;H<(A->Shape==EVoxelShapeKind::Door?2:1);++H)for(uint8 F=0;F<6;++F)
        {
            const auto& Face=(H?A->UpperFaceMaterials:A->FaceMaterials).Get(F);const FString K=KeyOf(A->RenderGroup,Face);
            if(Face.FrameCount>O.MaxSlicesPerBank){Error=TEXT("Animation exceeds bank slice limit: ")+K;return false;}
            if(!Lookup.Contains(K)){FSource S;S.Key=K;S.Face=Face;S.Group=A->RenderGroup;Lookup.Add(K,Sources.Add(MoveTemp(S)));}
        }
    }
    Sources.Sort([](const FSource&A,const FSource&B){return A.Key<B.Key;});Lookup.Reset();
    ...
    for(int32 I=0;I<Sources.Num();++I)
    {
        ...
        {Error=TEXT("Normalized texture source set exceeds 512 MiB; reduce tile/frame input");return false;}
        if(!ReadFrames(Sources[I],O.TileSize,Error))return false;
    ...
    TArray<FVoxelMaterialBank> Banks;TMap<uint8,UMaterial*> Masters;
    int32 Cursor=0;
    while(Cursor<Sources.Num())
    {
        ...
        {
            if(Bank>65535){Error=TEXT("Texture group exceeds 65536 banks");return false;}
            TArray<uint8> Pixels;int32 Count=0;
            while(Cursor<Sources.Num()&&Sources[Cursor].Group==G&&Count+Sources[Cursor].Face.FrameCount<=O.MaxSlicesPerBank)
            {
                auto& S=Sources[Cursor++];S.Baked.Bank=Bank;S.Baked.Layer=Count;S.Baked.Frames=S.Face.FrameCount;S.Baked.FPS=S.Face.FramesPerSecond;
                Count+=S.Face.FrameCount;Pixels.Append(S.Pixels);
            }
            ...
            Array->SRGB=true;Array->CompressionSettings=TC_Default;Array->MipGenSettings=TMGS_FromTextureGroup;
            Array->Filter=TF_Nearest;Array->AddressX=TA_Wrap;Array->AddressY=TA_Wrap;Array->AddressZ=TA_Clamp;
            Array->Source.Init(O.TileSize,O.TileSize,Count,1,TSF_BGRA8,Pixels.GetData());
            Array->PostEditChange();if(!FVoxelEditorAssetIO::Save(Array,Error))return false;
            UMaterial*& Master=Masters.FindOrAdd(uint8(G));
            if(!Master)Master=FVoxelMaterialBuilder::BuildMaster(BuildRoot+FString::Printf(TEXT("/M_G%u"),uint8(G)),G,Array,Error);
            if(!Master)return false;
            auto* MI=FVoxelMaterialBuilder::BuildInstance(Stem+TEXT("_MI"),Master,Array,Error);if(!MI)return false;
            FVoxelMaterialBank B;B.Group=G;B.Bank=Bank++;B.SliceCount=Count;B.Textures=Array;B.Material=MI;Banks.Add(B);
        }
    }
    // Resource generation is complete. The following saves form a source-control batch, not an OS transaction.
    for(UVoxelData* A:Assets)
    {
        A->Modify();A->BakedFaces.Reset();A->BakedUpperFaces.Reset();
        for(int32 H=0;H<(A->Shape==EVoxelShapeKind::Door?2:1);++H)for(uint8 F=0;F<6;++F)
        {
