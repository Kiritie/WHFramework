...
bool OnlyKeys(const FJsonObject& O, const TSet<FString>& Allowed)
{
	for (const auto& P : O.Values)
		if (!Allowed.Contains(FString(P.Key)))
			return false;
	return true;
}
...
{
	double N = 0;
	if (!V || !V->TryGetNumber(N) || !FMath::IsFinite(N) || N < Min || N > Max || std::floor(N) != N)
		return false;
	Out = int32(N);
	return true;
}
bool ReadAssetId(const FString& Text, FPrimaryAssetId& Out)
{
	FString Type;
	FString Name;
	if (!Text.Split(TEXT(":"), &Type, &Name) || Type.IsEmpty() || Name.IsEmpty() || Name.Contains(TEXT(":")))
		return false;
...
int32 UVoxelPrefabImportCommandlet::Main(const FString& Params)
{
	...
	double Version = 0;
	if (!FJsonSerializer::Deserialize(TJsonReaderFactory<>::Create(Text), Root) || !Root || !OnlyKeys(*Root, {TEXT("Version"), TEXT("Rows")}) ||
	    !Root->TryGetNumberField(TEXT("Version"), Version) || Version != 1 || !Root->TryGetArrayField(TEXT("Rows"), Rows) || Rows->IsEmpty() ||
	    Rows->Num() > 4096)
	{
	...
	for (int32 RowIndex = 0; RowIndex < Rows->Num(); ++RowIndex)
	{
		...
		if (!Row || !OnlyKeys(*Row, {TEXT("Asset"), TEXT("DisplayName"), TEXT("Cells")}) || !Row->TryGetStringField(TEXT("Asset"), AssetPath) ||
		    !Row->TryGetArrayField(TEXT("Cells"), Cells) || Cells->Num() > VoxelPrefab::MaxCellCount || SeenAssets.Contains(AssetPath))
		{
		...
		{
			if (!Row->TryGetStringField(TEXT("DisplayName"), DisplayName))
				return 8;
		...
		for (int32 CellIndex = 0; CellIndex < Cells->Num(); ++CellIndex)
		{
			...
			FString AssetId;
			if (!Cell || !OnlyKeys(*Cell, {TEXT("Offset"), TEXT("AssetId"), TEXT("State")}) || !Cell->TryGetArrayField(TEXT("Offset"), Offset) ||
			    Offset->Num() != 3 || !Cell->TryGetStringField(TEXT("AssetId"), AssetId) || !Cell->HasField(TEXT("State")))
			{
			...
			{
				if (!ReadInteger((*Offset)[Axis], -VoxelBlock::MaxAbsCoordinate + 1, VoxelBlock::MaxAbsCoordinate - 1, Out.Offset[Axis]))
				{
					UE_LOG(LogTemp, Error, TEXT("Invalid integer offset %d:%d"), RowIndex, CellIndex);
					return 10;
				}
			}
			if (!ReadInteger(Cell->TryGetField(TEXT("State")), 0, 65535, Out.Item.State) || !ReadAssetId(AssetId, Out.Item.VoxelAssetID))
			{
	...
	for (FStagedPrefab& Entry : Staged)
	{
		...
		if (Entry.bSetDisplayName)
			Entry.Asset->DisplayName = Entry.DisplayName;
		if (!FVoxelEditorAssetIO::Save(Entry.Asset.Get(), Error))
		{
			UE_LOG(
			    LogTemp, Error, TEXT("Prefab save failed after %d successful saves: %s. Keep the export and inspect source-control changes."), Saved, *Error);
			return 13;
		}
		++Saved;
	}
