...
UMaterial* FVoxelMaterialBuilder::BuildMaster(const FString& Path, EVoxelRenderGroup Group, UTexture2DArray* Array, FString& Error)
{
	if (!Array)
	{
		Error = TEXT("Material requires an array texture");
		return nullptr;
	}
	...
	auto* UV = Node<UMaterialExpressionTextureCoordinate>(M, -1000, 0);
	UV->CoordinateIndex = 0;
	auto* Layer = Node<UMaterialExpressionTextureCoordinate>(M, -1000, 160);
	Layer->CoordinateIndex = 1;
	auto* FPS = Node<UMaterialExpressionTextureCoordinate>(M, -1000, 300);
	FPS->CoordinateIndex = 2;
	...
	auto* Coord = Node<UMaterialExpressionCustom>(M, -650, 0);
	Coord->OutputType = CMOT_Float3;
	Coord->Description = TEXT("Voxel array layer, animation and water UV");
	Coord->Code = Group == EVoxelRenderGroup::Water ? TEXT("float2 p=UV+float2(T*0.035,T*0.019); return float3(p,L.x+fmod(floor(T*F.x),max(1.0,L.y)));")
	                                                : TEXT("return float3(UV,L.x+fmod(floor(T*F.x),max(1.0,L.y)));");
	AddInput(Coord, TEXT("UV"), UV);
	AddInput(Coord, TEXT("L"), Layer);
	AddInput(Coord, TEXT("F"), FPS);
	AddInput(Coord, TEXT("T"), Time);
	auto* Sample = Node<UMaterialExpressionTextureSampleParameter2DArray>(M, -350, 0);
	Sample->ParameterName = TEXT("VoxelArray");
	...
	auto* Rough = Node<UMaterialExpressionConstant>(M, -300, 360);
	Rough->R = Trans ? 0.12f : 0.85f;
	...
	if (Group == EVoxelRenderGroup::Emissive)
		OK &= UMaterialEditingLibrary::ConnectMaterialProperty(Sample, TEXT("RGB"), MP_EmissiveColor);
	if (Trans)
	{
		auto* Fade = Node<UMaterialExpressionDepthFade>(M, -600, 850);
		Fade->FadeDistanceDefault = 80.f;
		Fade->OpacityDefault = 1.f;
		auto* Fresnel = Node<UMaterialExpressionFresnel>(M, -600, 1050);
		auto* Opacity = Node<UMaterialExpressionCustom>(M, -250, 850);
		...
		AddInput(Opacity, TEXT("A"), Sample, 4);
		AddInput(Opacity, TEXT("D"), Fade);
		AddInput(Opacity, TEXT("F"), Fresnel);
	...
	{
		auto* Wind = Node<UMaterialExpressionCustom>(M, -250, 600);
		Wind->OutputType = CMOT_Float3;
		Wind->Code = Group == EVoxelRenderGroup::Foliage ? TEXT("float a=sin(T*1.8+P.x*0.008+P.y*0.006)*Mask*3.0;return float3(a,a*0.3,0);")
		                                                 : TEXT("return float3(0,0,sin(T*1.5+P.x*0.012)*sin(T*0.8+P.y*0.011)*1.0);");
		AddInput(Wind, TEXT("P"), Pos);
		AddInput(Wind, TEXT("T"), Time);
		AddInput(Wind, TEXT("Mask"), Color, 1);
		OK &= UMaterialEditingLibrary::ConnectMaterialProperty(Wind, TEXT(""), MP_WorldPositionOffset);
	}
	if (!OK)
	{
		Error = TEXT("Material graph connection failed: ") + Path;
		return nullptr;
	}
	M->PostEditChange();
...
UMaterialInstanceConstant* FVoxelMaterialBuilder::BuildInstance(const FString& Path, UMaterial* Parent, UTexture2DArray* Array, FString& Error)
{
	if (!Parent || !Array)
	{
		Error = TEXT("Missing material parent or texture array");
		return nullptr;
	}
