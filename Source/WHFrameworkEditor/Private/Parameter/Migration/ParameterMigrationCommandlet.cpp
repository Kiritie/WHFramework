...
void AddError(const FContext& Context, const FString& PropertyPath, const FParameter& Parameter, const FString& Error)
{
	...
	Entry->SetStringField(TEXT("OldType"),
						  StaticEnum<EParameterType>()->GetNameStringByValue(static_cast<int64>(Parameter.GetParameterType())));
...
void RecordParameter(const FContext& Context, const FString& PropertyPath, const FParameter& Parameter, UObject* Owner)
{
	...
	if (Context.RecordKeys.Contains(RecordKey))
		return;
	...
	static const TMap<EParameterType, FName> ValuePropertyNames = {{EParameterType::Integer, TEXT("IntegerValue")},
																   {EParameterType::Float, TEXT("FloatValue")},
																   {EParameterType::Byte, TEXT("ByteValue")},
																   {EParameterType::Enum, TEXT("EnumValue")},
																   {EParameterType::String, TEXT("StringValue")},
																   {EParameterType::Name, TEXT("NameValue")},
																   {EParameterType::Text, TEXT("TextValue")},
																   {EParameterType::Boolean, TEXT("BooleanValue")},
																   {EParameterType::Vector, TEXT("VectorValue")},
																   {EParameterType::Rotator, TEXT("RotatorValue")},
																   {EParameterType::Transform, TEXT("TransformValue")},
																   {EParameterType::Color, TEXT("ColorValue")},
																   {EParameterType::LinearColor, TEXT("LinearColorValue")},
																   {EParameterType::Key, TEXT("KeyValue")},
																   {EParameterType::Tag, TEXT("TagValue")},
																   {EParameterType::Tags, TEXT("TagsValue")},
																   {EParameterType::Brush, TEXT("BrushValue")},
																   {EParameterType::Guid, TEXT("GuidValue")},
																   {EParameterType::AssetID, TEXT("AssetIDValue")},
																   {EParameterType::Class, TEXT("ClassValue")},
																   {EParameterType::ClassPtr, TEXT("ClassPtrValue")},
																   {EParameterType::Object, TEXT("ObjectValue")},
																   {EParameterType::ObjectInst, TEXT("ObjectInstValue")},
																   {EParameterType::ObjectPtr, TEXT("ObjectPtrValue")},
	...
	{
		Entry->SetStringField(TEXT("Description"),
							  ExportProperty(DescriptionProperty, DescriptionProperty->ContainerPtrToValuePtr<void>(&Parameter), Owner));
	}
...
void RestoreParameter(FContext& Context, const FString& PropertyPath, FParameter& Parameter, UObject* Owner)
{
	...
	Context.RecordKeys.Add(RecordKey);
	if (Context.RestoredCount)
		++*Context.RestoredCount;
	if (Context.ModifiedPackages)
		Context.ModifiedPackages->Add(Owner->GetOutermost());
}
...
void ScanProperty(FProperty* Property, void* Value, const FString& Path, FContext& Context, UObject* Owner)
{
	if (FStructProperty* StructProperty = CastField<FStructProperty>(Property))
	{
		...
		{
			if (Context.Imports)
				RestoreParameter(Context, Path, *static_cast<FParameter*>(Value), Owner);
			else
				RecordParameter(Context, Path, *static_cast<FParameter*>(Value), Owner);
		}
		else
		{
	...
	if (FSetProperty* SetProperty = CastField<FSetProperty>(Property))
	{
		...
		for (int32 Index = 0; Index < Helper.GetMaxIndex(); ++Index)
		{
			...
			ScanProperty(SetProperty->ElementProp, Helper.GetElementPtr(Index),
						 FString::Printf(TEXT("%s{Element=%s}"), *Path, *ElementText.ReplaceCharWithEscapedChar()), Context, Owner);
		}
	}
}
...
int32 UParameterMigrationCommandlet::Main(const FString& Params)
{
	...
	FString Output = FPaths::ProjectSavedDir() /
					 (bAfter ? TEXT("FParameterMigration/FParameter_After.json") : TEXT("FParameterMigration/FParameter_Before.json"));
	...
	if (bImport || bAfter)
	{
		...
		for (const TSharedPtr<FJsonValue>& Value : BeforeDocument->GetArrayField(TEXT("Parameters")))
		{
			...
			const FString Key = Entry->GetStringField(TEXT("AssetPath")) + TEXT("|") + Entry->GetStringField(TEXT("ObjectPath")) +
								TEXT("|") + Entry->GetStringField(TEXT("PropertyPath"));
	...
	for (const FAssetData& Asset : Assets)
	{
		...
		for (UObject* Object : Objects)
		{
			if (!IsValid(Object) || Object->HasAnyFlags(RF_Transient))
				continue;
			ParameterMigration::FContext Context{Asset.PackageName.ToString(),
												 Object->GetPathName(),
												 Object->GetClass()->GetPathName(),
												 Records,
												 Errors,
												 RecordKeys,
												 bImport ? &Imports : nullptr,
												 &RestoredCount,
												 &ModifiedPackages};
			...
			if (UDataTable* DataTable = Cast<UDataTable>(Object))
			{
				if (const UScriptStruct* RowStruct = DataTable->GetRowStruct())
				{
					...
					{
						ParameterMigration::ScanStruct(RowStruct, Row.Value, FString::Printf(TEXT("Rows[%s]"), *Row.Key.ToString()),
													   Context, DataTable);
					}
				}
			}
	...
	if (bImport)
	{
		...
		UE_LOG(LogTemp, Display, TEXT("Restored %d/%d FParameter values; packages=%d errors=%d"), RestoredCount, Imports.Num(),
			   ModifiedPackages.Num(), Errors.Num() + FailedAssets);
	...
	Records.Sort(
		[](const TSharedPtr<FJsonValue>& A, const TSharedPtr<FJsonValue>& B)
		{
			...
			return AO->GetStringField(TEXT("AssetPath")) + AO->GetStringField(TEXT("ObjectPath")) +
					   AO->GetStringField(TEXT("PropertyPath")) <
				   BO->GetStringField(TEXT("AssetPath")) + BO->GetStringField(TEXT("ObjectPath")) +
					   BO->GetStringField(TEXT("PropertyPath"));
		});
	...
	if (bAfter)
	{
		for (const TSharedPtr<FJsonValue>& Value : Records)
		{
			...
			if (!BeforeEntry ||
				(*BeforeEntry)->GetStringField(TEXT("OldParameterType")) != Entry->GetStringField(TEXT("OldParameterType")) ||
		...
		if (Records.Num() != Imports.Num())
		{
			...
			Error->SetStringField(TEXT("Error"),
								  FString::Printf(TEXT("Parameter count mismatch: Before=%d After=%d"), Imports.Num(), Records.Num()));
			Errors.Add(MakeShared<FJsonValueObject>(Error));
		}
		Document->SetNumberField(TEXT("ErrorCount"), Errors.Num());
		Document->SetArrayField(TEXT("Errors"), Errors);
	...
	if (!FFileHelper::SaveStringToFile(Json, *Output, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM))
		return 1;
	UE_LOG(LogTemp, Display, TEXT("Recorded %d FParameter values from %d assets with %d errors: %s"), Records.Num(), Assets.Num(),
		   Errors.Num() + FailedAssets, *Output);
