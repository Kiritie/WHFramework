...
FString FPlatformManager::GetEngineDescription(const FString& Identifier)
{
...
FString FPlatformManager::GetCurrentEngineIdentifier()
{
...
void FPlatformManager::EnumerateLauncherEngineInstallations(TMap<FString, FString> &OutInstallations)
{
...
void FPlatformManager::EnumerateLauncherSampleInstallations(TArray<FString> &OutInstallations)
{
	// Cache the launcher install list if necessary
	ReadLauncherInstallationList();
...
void FPlatformManager::EnumerateLauncherSampleProjects(TArray<FString> &OutFileNames)
{
...
bool FPlatformManager::GetEngineRootDirFromIdentifier(const FString &Identifier, FString &OutRootDir)
{
...
bool FPlatformManager::GetEngineIdentifierFromRootDir(const FString &RootDir, FString &OutIdentifier)
{
	// Get all the installations
	TMap<FString, FString> Installations;
	EnumerateEngineInstallations(Installations);
...
bool FPlatformManager::GetDefaultEngineIdentifier(FString &OutId)
{
	TMap<FString, FString> Installations;
	EnumerateEngineInstallations(Installations);
...
bool FPlatformManager::GetDefaultEngineRootDir(FString &OutDirName)
{
	FString Identifier;
	return GetDefaultEngineIdentifier(Identifier) && GetEngineRootDirFromIdentifier(Identifier, OutDirName);
}

bool FPlatformManager::IsPreferredEngineIdentifier(const FString &Identifier, const FString &OtherIdentifier)
{
...
bool FPlatformManager::TryGetEngineVersion(const FString& RootDir, FEngineVersion& OutVersion)
{
...
bool FPlatformManager::IsStockEngineRelease(const FString &Identifier)
{
	FGuid Guid;
	return !FGuid::Parse(Identifier, Guid);
}

bool FPlatformManager::TryParseStockEngineVersion(const FString& Identifier, FEngineVersion& OutVersion)
{
	TCHAR* End;
...
bool FPlatformManager::IsSourceDistribution(const FString &EngineRootDir)
{
...
bool FPlatformManager::IsPerforceBuild(const FString &EngineRootDir)
{
	// Check for the existence of a SourceBuild.txt file
...
bool FPlatformManager::IsValidRootDirectory(const FString &RootDir)
{
...
bool FPlatformManager::SetEngineIdentifierForProject(const FString &ProjectFileName, const FString &InIdentifier)
{
...
bool FPlatformManager::GetEngineIdentifierForProject(const FString& ProjectFileName, FString& OutIdentifier)
{
	OutIdentifier.Empty();
...
bool FPlatformManager::OpenProject(const FString& ProjectFileName)
{
	FPlatformProcess::LaunchFileInDefaultExternalApplication(*ProjectFileName);
	return true;
}

bool FPlatformManager::CleanGameProject(const FString& ProjectDir, FString& OutFailPath, FFeedbackContext* Warn)
{
	// Begin a task
...
bool FPlatformManager::CompileGameProject(const FString& RootDir, const FString& ProjectFileName, FFeedbackContext* Warn, ECompilationResult::Type* OutResult)
{
	...
	// Run UBT
	int32 ExitCode;
	bool bResult = static_cast<IPlatformManager*>(this)->RunUnrealBuildTool(LOCTEXT("CompilingProject", "Compiling project..."), RootDir, Arguments, Warn, ExitCode);
...
bool FPlatformManager::GenerateProjectFiles(const FString& RootDir, const FString& ProjectFileName, FFeedbackContext* Warn, FString LogFilePath)
{
...
bool FPlatformManager::IsUnrealBuildToolAvailable()
{
...
bool FPlatformManager::InvokeUnrealBuildToolSync(const FString& InCmdLineParams, FOutputDevice &Ar, bool bSkipBuildUBT, int32& OutReturnCode, FString& OutProcOutput)
{
...
FProcHandle FPlatformManager::InvokeUnrealBuildToolAsync(const FString& InCmdLineParams, FOutputDevice &Ar, void*& OutReadPipe, void*& OutWritePipe, bool bSkipBuildUBT)
{
...
bool FPlatformManager::RunUnrealBuildTool(const FText& Description, const FString& RootDir, const FString& Arguments, FFeedbackContext* Warn)
{
	int32 ExitCode;
	return static_cast<IPlatformManager*>(this)->RunUnrealBuildTool(Description, RootDir, Arguments, Warn, ExitCode);
}
...
bool IsTargetInfoValidEx(const TArray<FTargetInfo>& Targets, const FString& SourceDir, const FDateTime& LastModifiedTime)
{
	// Create the state 
	TSet<FString> RemainingTargetNames;
...
const TArray<FTargetInfo>& FPlatformManager::GetTargetsForProject(const FString& ProjectFile) const
{
	...
	if(StatData.bIsValid)
	{
		...
		if(ReadTargetInfo(InfoFileName, NewTargets) && IsTargetInfoValidEx(NewTargets, ProjectSourceDir, StatData.ModificationTime))
		{
	...
	FString Output;
	int32 ReturnCode = 0;
	if (!FPaths::FileExists(*GetUnrealBuildToolExecutableFilename(FPaths::RootDir())) || !const_cast<FPlatformManager*>(this)->InvokeUnrealBuildToolSync(Arguments, *GLog, true, ReturnCode, Output) || ReturnCode != 0)
	{
		...
		if (!GIsBuildMachine)
		{
			const_cast<FPlatformManager*>(this)->InvokeUnrealBuildToolSync(Arguments, *GLog, false, ReturnCode, Output);
		}
	}
	...
	{
		WHLog(FString::Printf(TEXT("Unable to read target info for %s"), (ProjectFile.Len() == 0)? TEXT("engine") : *ProjectFile), EDC_Platform, EDV_Warning);
	}
...
const TArray<FTargetInfo>& FPlatformManager::GetTargetsForCurrentProject() const
{
	return GetTargetsForProject(FPaths::GetProjectFilePath());
}

bool FPlatformManager::GetSolutionPath(FString& OutSolutionPath)
{
...
FString FPlatformManager::GetDefaultProjectCreationPath()
{
	// My Documents
...
void FPlatformManager::ReadLauncherInstallationList()
{
...
void FPlatformManager::CheckForLauncherEngineInstallation(const FString &AppId, const FString &Identifier, TMap<FString, FString> &OutInstallations)
{
...
int32 FPlatformManager::ParseReleaseVersion(const FString &Version)
{
	TCHAR *End;
...
TSharedPtr<FJsonObject> FPlatformManager::LoadProjectFile(const FString &FileName)
{
...
bool FPlatformManager::SaveProjectFile(const FString &FileName, TSharedPtr<FJsonObject> Object)
{
	FString FileContents;
...
const FUProjectDictionary &FPlatformManager::GetCachedProjectDictionary(const FString& RootDir)
{
	FString NormalizedRootDir = RootDir;
	FPaths::NormalizeDirectoryName(NormalizedRootDir);
...
void FPlatformManager::GetProjectBuildProducts(const FString& ProjectDir, TArray<FString> &OutFileNames, TArray<FString> &OutDirectoryNames)
{
...
FString FPlatformManager::GetEngineSavedConfigDirectory(const FString& Identifier)
{
...
bool FPlatformManager::EnumerateProjectsKnownByEngine(const FString &Identifier, bool bIncludeNativeProjects, TArray<FString> &OutProjectFileNames)
{
	// Get the engine root directory
	FString RootDir;
	if (!GetEngineRootDirFromIdentifier(Identifier, RootDir))
...
bool FPlatformManager::BuildUnrealBuildTool(const FString& RootDir, FOutputDevice& Ar)
{
...
bool FPlatformManager::ReadTargetInfo(const FString& FileName, TArray<FTargetInfo>& Targets)
{
	// Read the file to a string
	FString Contents;
	...
	for (int Idx = 0; Idx < TargetArray->Num(); Idx++)
	{
		...
		FString TempType;
		if (!TargetObject.TryGetStringField(TEXT("Type"), TempType))
		{
			return false;
		}
		if(!LexTryParseString(Targets[Idx].Type, *TempType) || Targets[Idx].Type == EBuildTargetType::Unknown)
		{
			return false;
		}
	}
...
FString FPlatformManager::GetUnrealBuildToolProjectFileName(const FString& RootDir) const
{
	return FPaths::ConvertRelativePathToFull(RootDir / TEXT("Engine/Source/Programs/UnrealBuildTool/UnrealBuildTool.csproj"));
}

FString FPlatformManager::GetUnrealBuildToolExecutableFilename(const FString& RootDir) const
{
