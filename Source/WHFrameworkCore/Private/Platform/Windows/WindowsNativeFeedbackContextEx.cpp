...
FWindowsNativeFeedbackContextEx::FWindowsNativeFeedbackContextEx()
	: FFeedbackContext()
	, Context( NULL )
	, hThread( NULL )
	, hCloseEvent( NULL )
...
void FWindowsNativeFeedbackContextEx::Serialize( const TCHAR* V, ELogVerbosity::Type Verbosity, const class FName& Category )
{
...
bool FWindowsNativeFeedbackContextEx::ReceivedUserCancel()
{
	FScopeLock Lock(&CriticalSection);
	return bReceivedUserCancel;
}

void FWindowsNativeFeedbackContextEx::StartSlowTask( const FText& Task, bool bShouldShowCancelButton )
{
	FFeedbackContext::StartSlowTask( Task, bShouldShowCancelButton );
	CreateSlowTaskWindow(Task, bShouldShowCancelButton);
}
void FWindowsNativeFeedbackContextEx::FinalizeSlowTask( )
{
	FFeedbackContext::FinalizeSlowTask( );
	DestroySlowTaskWindow();
}

void FWindowsNativeFeedbackContextEx::ProgressReported( const float TotalProgressInterp, FText DisplayMessage )
{
	FScopeLock Lock(&CriticalSection);
	if(hThread != NULL)
	{
...
FContextSupplier* FWindowsNativeFeedbackContextEx::GetContext() const
{
	return Context;
}

void FWindowsNativeFeedbackContextEx::SetContext( FContextSupplier* InSupplier )
{
	Context = InSupplier;
}

void FWindowsNativeFeedbackContextEx::CreateSlowTaskWindow(const FText &InStatus, bool bInShowCancelButton)
{
	FScopeLock Lock(&CriticalSection);
...
void FWindowsNativeFeedbackContextEx::DestroySlowTaskWindow()
{
	FScopeLock Lock(&CriticalSection);
	if(hThread != NULL)
	{
...
DWORD FWindowsNativeFeedbackContextEx::SlowTaskThreadProc(void* ThreadParam)
{
	FWindowsNativeFeedbackContextEx* Context = (FWindowsNativeFeedbackContextEx*)ThreadParam;
...
LRESULT CALLBACK FWindowsNativeFeedbackContextEx::SlowTaskWindowProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	switch (Msg)
	{
	case WM_COMMAND:
...
void FWindowsNativeFeedbackContextEx::LayoutControls(HWND hWnd, const FWindowParams* Params)
{
	RECT ClientRect;
