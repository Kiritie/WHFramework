...
void FFileDownloader::DownloadFile(const FString& SavePath, float Timeout, const FString& ContentType, bool bForceByPayload)
{
	...
	FString URL = FCoreStatics::UrlEncode((GetFileURL().FileURL.StartsWith(TEXT("/")) ? FWebRequestManager::Get().GetServerURL() : TEXT("")) + GetFileURL().FileURL);
	URL = URL.Replace(TEXT("%2F"), TEXT("/")).Replace(TEXT("%3A"), TEXT(":")).Replace(TEXT("+"), TEXT("%20"));

	if (HttpRequestPtr.IsValid())
	{
		WHLog(TEXT("The downloader has started"), EDC_WebRequest, EDV_Warning);
		return;
	}
	...
	FileHandle = PlatformFile.OpenWrite(*FileSavePath);
	if (!FileHandle)
	{
	...
	auto OnResult = [this, WeakThisPtr](FFileDownloaderResult&& Result) mutable
	{
		TSharedPtr<FFileDownloader> SharedThis = WeakThisPtr.Pin();

		if(SharedThis.IsValid() && SharedThis->DownloadResult == EDownloadToStorageResult::None)
		{
			SharedThis->HttpRequestPtr.Reset();

			SharedThis->DeleteFileHandle();
	...
	{
		// int64 MaxChunkSize = TNumericLimits<TArray<uint8>::SizeType>::Max();
		int64 MaxChunkSize = BytesOfMB * 100;
		DownloadFile(URL, Timeout, ContentType, MaxChunkSize).Next(OnResult);
	}
}
...
TFuture<FFileDownloaderResult> FFileDownloader::DownloadFile(const FString& URL, float Timeout, const FString& ContentType, int64 MaxChunkSize)
{
	...
	GetContentSize(URL, Timeout).Next([WeakThisPtr, PromisePtr, URL, Timeout, ContentType, MaxChunkSize](int64 ContentSize) mutable
	{
		...
		{
			WHLog(FString::Printf(TEXT("Failed to download file InChunk from %s: downloader has been destroyed"), *URL), EDC_WebRequest, EDV_Warning);
		...
		auto DownloadByPayload = [SharedThis, WeakThisPtr, PromisePtr, URL, Timeout, ContentType]()
		{
			SharedThis->DownloadFileByPayload(URL, Timeout, ContentType).Next([WeakThisPtr, PromisePtr, URL, Timeout, ContentType](FFileDownloaderResult Result) mutable
			{
				...
				if (SharedThis->DownloadResult == EDownloadToStorageResult::Cancelled)
				{
					WHLog(FString::Printf(TEXT("Canceled file InChunk download from %s"), *URL), EDC_WebRequest, EDV_Warning);
					PromisePtr->SetValue(FFileDownloaderResult{EDownloadToMemoryResult::Cancelled, TArray64<uint8>()});
					return;
				}

				if (!SharedThis->FileHandle->Write(Result.Data.GetData(), Result.Data.Num()))
				{
					WHLog(FString::Printf(TEXT("Something went wrong while writing the response data to the file '%s'"), *SharedThis->FileSavePath), EDC_WebRequest, EDV_Error);
					delete SharedThis->FileHandle;
					SharedThis->OnComplete.Broadcast(SharedThis->DownloadResult = EDownloadToStorageResult::SaveFailed, SharedThis->FileSavePath);
				}

				PromisePtr->SetValue(FFileDownloaderResult{Result.Result, TArray64<uint8>()/*MoveTemp(Result.Data)*/});
			});
		};
		...
		if (MaxChunkSize <= 0)
		{
			WHLog(FString::Printf(TEXT("Failed to download file InChunk from %s: MaxChunkSize is <= 0. Trying to download the file by payload"), *URL), EDC_WebRequest, EDV_Error);
			DownloadByPayload();
			return;
		}
		...
		auto OnChunkDownloaded = [WeakThisPtr, PromisePtr, URL, ContentSize, Timeout, ContentType, DownloadByPayload/*, OverallDownloadedDataPtr*/, bChunkDownloadedFilledPtr, ChunkOffsetPtr, OnChunkDownloadedFilled](TArray64<uint8>&& ResultData) mutable
		{
			...
			// 	{
			// 		WHLog(FString::Printf(TEXT("Failed to download file InChunk from %s: data offset is out of range (%lld, expected [0, %lld]). Trying to download the file by payload"), *URL, *ChunkOffsetPtr, OverallDownloadedDataPtr->Num()), EDC_WebRequest, EDV_Error);
			// 		DownloadByPayload();
			// 		OnChunkDownloadedFilled();
			// 		return;
			// 	}
			...
			// 	{
			// 		WHLog(FString::Printf(TEXT("Failed to download file InChunk from %s: overall downloaded size is out of range (%lld, expected [0, %lld]). Trying to download the file by payload"), *URL, CurrentlyDownloadedSize, OverallDownloadedDataPtr->Num()), EDC_WebRequest, EDV_Error);
			// 		DownloadByPayload();
			// 		OnChunkDownloadedFilled();
			// 		return;
			// 	}
			...
			// Append the downloaded InChunk to the result data
			// FMemory::Memcpy(OverallDownloadedDataPtr->GetData() + *ChunkOffsetPtr, ResultData.GetData(), ResultData.Num());

			if (!SharedThis->FileHandle->Write(ResultData.GetData(), ResultData.Num()))
			{
				WHLog(FString::Printf(TEXT("Something went wrong while writing the response data to the file '%s'"), *SharedThis->FileSavePath), EDC_WebRequest, EDV_Error);
				delete SharedThis->FileHandle;
				SharedThis->OnComplete.Broadcast(SharedThis->DownloadResult = EDownloadToStorageResult::SaveFailed, SharedThis->FileSavePath);
			...
			{
				PromisePtr->SetValue(FFileDownloaderResult{EDownloadToMemoryResult::Success, TArray64<uint8>()/*MoveTemp(*OverallDownloadedDataPtr.Get())*/});
				OnChunkDownloadedFilled();
				return;
			}

			// Increase the offset by the size of the downloaded InChunk
		...
		{
			// Only return data if no InChunk was downloaded
			if (bChunkDownloadedFilledPtr.IsValid() && (*bChunkDownloadedFilledPtr.Get() == false))
			{
				...
				{
					WHLog(FString::Printf(TEXT("Failed to download file InChunk from %s: download failed. Trying to download the file by payload"), *URL), EDC_WebRequest, EDV_Error);
					DownloadByPayload();
					OnChunkDownloadedFilled();
					return;
				}
				// OverallDownloadedDataPtr->Shrink();
				PromisePtr->SetValue(FFileDownloaderResult{Result, TArray64<uint8>()/*MoveTemp(*OverallDownloadedDataPtr.Get())*/});
			}
		});
	});
	...
	GetContentSize(URL, Timeout).Next([WeakThisPtr, PromisePtr, URL, Timeout, ContentType, MaxChunkSize, OnChunkDownloaded, ChunkRange](int64 ContentSize) mutable
	{
		...
		if (ContentSize <= 0)
		{
			...
			SharedThis->DownloadFileByPayload(URL, Timeout, ContentType).Next([WeakThisPtr, PromisePtr, URL, Timeout, ContentType, OnChunkDownloaded](FFileDownloaderResult Result) mutable
			{
				...
				{
					WHLog(FString::Printf(TEXT("Failed to download file InChunk from %s: %s"), *URL, *UEnum::GetValueAsString(Result.Result)), EDC_WebRequest, EDV_Error);
				...
				{
					WHLog(FString::Printf(TEXT("Failed to download file InChunk from %s: downloaded content is empty"), *URL), EDC_WebRequest, EDV_Error);
					PromisePtr->SetValue(EDownloadToMemoryResult::DownloadFailed);
					return;
				}
		...
		if (MaxChunkSize <= 0)
		{
			WHLog(FString::Printf(TEXT("Failed to download file InChunk from %s: max InChunk size is <= 0"), *URL), EDC_WebRequest, EDV_Error);
			PromisePtr->SetValue(EDownloadToMemoryResult::DownloadFailed);
			return;
		}

		// If the InChunk range is not specified, determine the range based on the max InChunk size and the content size
		...
		{
			WHLog(FString::Printf(TEXT("Failed to download file InChunk from %s: InChunk range is out of range (%lld, expected [0, %lld])"), *URL, ChunkRange.Y, ContentSize), EDC_WebRequest, EDV_Error);
			PromisePtr->SetValue(EDownloadToMemoryResult::DownloadFailed);
			return;
		}
		...
		auto OnProgressInternal = [WeakThisPtr, URL, ChunkRange](int64 BytesSent, int64 BytesReceived, int64 ContentSize) mutable
		{
			...
			if (SharedThis.IsValid())
			{
				...
				WHLog(FString::Printf(TEXT("Downloaded %lld bytes of file InChunk from %s. Range: {%lld; %lld}, Overall: %lld, Progress: %f"), BytesReceived, *URL, ChunkRange.X, ChunkRange.Y, ContentSize, Progress), EDC_WebRequest, EDV_Log);
				SharedThis->OnProgress_Internal(URL, 0, 1, BytesSent, BytesReceived + ChunkRange.X, ContentSize);
				SharedThis->OnProgress.Broadcast(URL, 0, 1, BytesSent, BytesReceived + ChunkRange.X, ContentSize);
			}
		};
...
TFuture<FFileDownloaderResult> FFileDownloader::DownloadFileByChunk(const FString& URL, float Timeout, const FString& ContentType, int64 ContentSize, FInt64Vector2 ChunkRange, const TFunction<void(int64, int64, int64)>& ProgressFun)
{
	...
	{
		WHLog(FString::Printf(TEXT("Failed to download file InChunk from %s: InChunk range (%lld; %lld) is invalid"), *URL, ChunkRange.X, ChunkRange.Y), EDC_WebRequest, EDV_Error);
	...
	{
		WHLog(FString::Printf(TEXT("Failed to download file InChunk from %s: InChunk range (%lld; %lld) is out of range (%lld)"), *URL, ChunkRange.X, ChunkRange.Y, ContentSize), EDC_WebRequest, EDV_Error);
		return MakeFulfilledPromise<FFileDownloaderResult>(FFileDownloaderResult{EDownloadToMemoryResult::DownloadFailed, TArray64<uint8>()}).GetFuture();
	}
	...
	HttpRequestRef->OnProcessRequestComplete().BindLambda([WeakThisPtr, PromisePtr, URL, ChunkRange](FHttpRequestPtr Request, FHttpResponsePtr Response, bool bSuccess) mutable
	{
		...
		{
			WHLog(FString::Printf(TEXT("Failed to download file InChunk from %s: request failed"), *Request->GetURL()), EDC_WebRequest, EDV_Error);
			PromisePtr->SetValue(FFileDownloaderResult{EDownloadToMemoryResult::DownloadFailed, TArray64<uint8>()});
			return;
		}
		...
		// {
		// 	WHLog(FString::Printf(TEXT("Failed to download file InChunk from %s: content length is 0"), *Request->GetURL()), EDC_WebRequest, EDV_Error);
		// 	PromisePtr->SetValue(FFileDownloaderResult{EDownloadToMemoryResult::DownloadFailed, TArray64<uint8>()});
		// 	return;
		// }
		...
		{
			WHLog(FString::Printf(TEXT("Failed to download file InChunk from %s: content length (%lld) does not match the expected length (%lld)"), *Request->GetURL(), ContentLength, ChunkRange.Y - ChunkRange.X + 1), EDC_WebRequest, EDV_Error);
			PromisePtr->SetValue(FFileDownloaderResult{EDownloadToMemoryResult::DownloadFailed, TArray64<uint8>()});
			return;
		}

		WHLog(FString::Printf(TEXT("Successfully downloaded file InChunk from %s. Range: {%lld; %lld}, Overall: %lld"), *Request->GetURL(), ChunkRange.X, ChunkRange.Y, ContentLength), EDC_WebRequest, EDV_Log);
	...
	{
		WHLog(FString::Printf(TEXT("Failed to download file InChunk from %s: request failed"), *URL), EDC_WebRequest, EDV_Error);
		return MakeFulfilledPromise<FFileDownloaderResult>(FFileDownloaderResult{EDownloadToMemoryResult::DownloadFailed, TArray64<uint8>()}).GetFuture();
	}
...
TFuture<int64> FFileDownloader::GetContentSize(const FString& URL, float Timeout)
{
	...
	HttpRequestRef->OnProcessRequestComplete().BindLambda([WeakThisPtr, PromisePtr, URL](const FHttpRequestPtr& Request, const FHttpResponsePtr& Response, const bool bSucceeded)
	{
		TSharedPtr<FFileDownloader> SharedThis = WeakThisPtr.Pin();
		if (!SharedThis.IsValid())
		{
			WHLog(FString::Printf(TEXT("Failed to get size of file from %s: downloader has been destroyed"), *URL), EDC_WebRequest, EDV_Warning);
...
void FFileDownloader::DeleteFileHandle()
{
	if(FileHandle)
	{
		delete FileHandle;
...
void FFileDownloader::OnProgress_Internal(FString CurrentFileUrl, int64 CurrentFileIndex, int64 TotalFileNum, int64 BytesSent, int64 BytesReceived, int64 FullSize)
{
	IFileDownloaderInterface::OnProgress_Internal(CurrentFileUrl, CurrentFileIndex, TotalFileNum, BytesSent, BytesReceived, FullSize);
}
...
void FFileDownloader::OnComplete_Internal(EDownloadToMemoryResult Result, TArray64<uint8> DownloadedContent)
{
	...
	// 		[this, FileHandle, DownloadedContent]()
	// 		{
	// 			FileHandle->Write(DownloadedContent.GetData(), DownloadedContent.Num());
	// 			DeleteFileHandle();
	// 			OnComplete.Broadcast(DownloadResult = EDownloadToStorageResult::SaveFailed, FileSavePath);
	// 		}
	// 	);
	// }
	// else if (!FileHandle->Write(DownloadedContent.GetData(), DownloadedContent.Num()))
	// {
