...
struct TParameterValueAdapter<T, std::enable_if_t<std::is_base_of_v<FParameterValueBase, T>>>
{
	...
	template<typename U>
	static void Set(TInstancedStruct<FParameterValueBase>& OutValue, U&& InValue)
	{
		OutValue.template InitializeAs<T>(Forward<U>(InValue));
	}

	static const T* GetPtr(const TInstancedStruct<FParameterValueBase>& InValue) { return InValue.template GetPtr<T>(); }
	static T* GetMutablePtr(TInstancedStruct<FParameterValueBase>& InValue) { return InValue.template GetMutablePtr<T>(); }
};
...
#define WH_PARAMETER_ADAPTER(ValueType, ParameterWrapper) \
	...
	{ \
		...
		{ const WrapperType* Wrapper = InValue.template GetPtr<WrapperType>(); return Wrapper ? &Wrapper->Value : nullptr; } \
		static ValueType* GetMutablePtr(TInstancedStruct<FParameterValueBase>& InValue) \
		{ WrapperType* Wrapper = InValue.template GetMutablePtr<WrapperType>(); return Wrapper ? &Wrapper->Value : nullptr; } \
	};
...
template<> struct TParameterValueAdapter<void*>
{
	...
	{ const FParameterPointerValue* Wrapper = InValue.GetPtr<FParameterPointerValue>(); return Wrapper ? &Wrapper->Value : nullptr; }
	static void** GetMutablePtr(TInstancedStruct<FParameterValueBase>& InValue)
	{ FParameterPointerValue* Wrapper = InValue.GetMutablePtr<FParameterPointerValue>(); return Wrapper ? &Wrapper->Value : nullptr; }
};
...
template<typename T>
struct TParameterValueAdapter<T*, std::enable_if_t<!std::is_base_of_v<UObject, std::remove_cv_t<T>>>>
{
	using WrapperType = FParameterPointerValue;
	static constexpr bool bSupported = true;
...
template<typename T> struct TParameterValueAdapter<T*, std::enable_if_t<std::is_base_of_v<UObject, T>>> : TParameterValueAdapter<FParameterObjectValue>
{
	template<typename U> static void Set(TInstancedStruct<FParameterValueBase>& OutValue, U&& InValue)
